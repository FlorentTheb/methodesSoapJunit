...
public class Methode4Test {
	private String testCaseName;

	public Methode4Test(String testCaseName) {
	...
	public static boolean runSoapUITestCase(String testCase) throws XmlException, IOException, SoapUIException {
		...
		for (TestSuite suite : testSuites) {
			System.out.println(
					"[SEARCH] recherche du TC [" + testCase + "] dans la suite " + suite.getName().toUpperCase());
			TestCase soapuiTestCase = suite.getTestCaseByName(testCase);
			if (soapuiTestCase == null) {
				System.out.println("[NOT FOUND YET] runner soapUI, le cas de test [" + testCase
						+ "] n'est pas dans cette suite de test");
			} else {
				...
				if (exitValue == TestRunner.Status.FINISHED) {
					System.out.println("[END] : Cas de test soapUI termin̩ ('" + suite.getName().toUpperCase() + "':["
							+ testCase + "]) : " + exitValue);
					return true;
				}
			}
